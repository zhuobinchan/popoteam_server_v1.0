 ...
 * 同市不同类型
 */
public class SameProvinceDifTypeMatcher extends RedisMatcher {
    private final Logger logger = Logger.getLogger(SameCitySameTypeMatcher.class);
    private final int MATCHER_RULE_INDEX = 4;
    @Override
    public boolean matchGroup(Long groupId, Long userId, Date time) {
        logger.info("==========SameProvinceDifTypeMatcher,Thread name: " + Thread.currentThread().getName() +", groupId: " + groupId);
        Group mainGroup = groupService.getGroupById(groupId);
        // 当队伍已经解散直接返回false
        if(mainGroup.getStatus().equals(GroupTypeAndStatus.GROUP_DISSOLUTION)){
            logger.info("==========SameProvinceDifTypeMatcher, " + groupId + " is dissolution =================");
        ...
        if(!checkRuleIfOpen(MATCHER_RULE_INDEX)){
            logger.info("==========SameProvinceDifTypeMatcher, close =================");
            return this.nextMatchGroup(groupId, userId, time);
        }

        Map<String, Object> params = new HashMap<>();
        params.put("provinceId", mainGroup.getProvinceId() == null ? " " : mainGroup.getProvinceId());
