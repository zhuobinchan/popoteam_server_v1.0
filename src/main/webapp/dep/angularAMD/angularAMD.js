...
define(function () {
    ...
    function checkBootstrapped() {
        if ( !bootstrapped ) {
            throw new Error('angularAMD not initialized.  Need to call angularAMD.bootstrap(ctrl) first.');
        }
    }
     ...
     * object into the current ng-ctrl.  As result, if there are subsequent call to retrieve the
     * module post processQueue, it would retrieve a module that is not integrated into the ng-ctrl.
     * 
    ...
    /**
     * Expose name of the ctrl that has been bootstrapped
     */
    ...
    /**
     * Recreate the modules created by alternate angular in ng-ctrl using cached $provider.
     ...
     * Designed to simulate ctrl.config.
     */
     ...
     * $provider and $injector from the ctrl to be used later.
     *
     * enable_ngload: 
     */
    AngularAMD.prototype.bootstrap = function (app, enable_ngload, elem) {
        ...
        // Store reference to original angular and ctrl
        orig_angular = angular;

        // Create new version of ctrl
        orig_app = app;
        alt_app = {};
        ...
        app.config(
            ['$controllerProvider', '$compileProvider', '$filterProvider', '$animateProvider', '$provide', '$injector', function (controllerProvider, compileProvider, filterProvider, animateProvider, provide, injector) {
                ...
                // Substitue provider methods from ctrl call the cached provider
        ...
        // Store the ctrl name needed by .bootstrap function.
        app_name = app.name;
        ...
        // Create a ctrl.register object to keep backward compatibility
