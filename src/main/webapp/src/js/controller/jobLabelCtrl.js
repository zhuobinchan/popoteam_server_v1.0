 ...
 */
define(['app', 'service/jobLabelServ'], function (app) {

    app.controller('JobLabelCtrl', JobLabelCtrl);

});

JobLabelCtrl.$inject = ['$scope','JobLabelServ'];

function JobLabelCtrl($scope,JobLabelServ) {
    ...
    getLabels(1, $scope.jobGridOpts.paginationPageSize);

    function getLabels(page, size) {
        $scope.isLoading = true;
        JobLabelServ.getJobLabelsInPages(page, size, $scope.params).success(function (data) {
            if (200 == data.code) {
                $scope.jobGridOpts.totalItems = data.totalSize;
                $scope.jobGridOpts.data = data.list;
    ...
    function chgLabelsStuta(isOpen) {
        var selectedItems = $scope.jobGridApi.selection.getSelectedRows();
        if (selectedItems.length <= 0) {
            swal(GCONSTANTS.MSG.NO_SELECTED);
            return;
        }
        var ids = _(selectedItems).pluck('id');
        JobLabelServ.setJobLabelStatus(ids, isOpen).success(function (data) {
            if (200 == data.code) {
    ...
    function saveLabel() {
        ...
        if (submitObj.id) {//编辑
            JobLabelServ.updateJobLabel(submitObj).success(function (data) {
                if (200 == data.code) {
                    swal({
                        title: '编辑成功',
                        type: 'success',
            ...
            JobLabelServ.addJobLabel(submitObj).success(function (data) {
                if (200 == data.code) {
                    swal({
                        title: '新建成功',
                        type: 'success',
